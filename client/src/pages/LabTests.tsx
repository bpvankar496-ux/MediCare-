...
export default function LabTests() {
  ...
  const bookTests = async () => {
    ...
    refetchBookings()
    showToast(t('lt_toast_confirmed'), 'success')
  }
    ...
    <div className="fade-in">
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: '1 1 200px' }}>
          ...
          <input className="input" style={{ paddingLeft: 40 }} placeholder={t('lt_search_placeholder')} value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <select className="input" style={{ width: 'auto' }} value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}>
          {categories.map((c) => <option key={c} value={c}>{c === 'all' ? t('lt_all_categories') : c}</option>)}
        </select>
        {totalTests > 0 && (
          <button className="btn btn-primary" onClick={() => setCheckoutOpen(true)}>
            <Calendar size={18} /> {t('lt_book_tests')} {totalTests} {totalTests > 1 ? t('lt_test_plural') : t('lt_test_singular')} - ₹{total}
          </button>
        )}
      </div>
          ...
          return (
            <div key={test.id} className="card" style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', fontSize: 12, color: 'var(--text-muted)' }}>
                {test.fasting_required && <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Clock size={12} /> {t('lt_fasting')}</span>}
                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Droplet size={12} /> {test.sample_type}</span>
                <span>{t('lt_report')} {test.report_time}</span>
              </div>
              ...
              {qty > 0 ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--primary-50)', borderRadius: 'var(--radius-sm)', padding: '4px 8px' }}>
                  ...
                  <span style={{ fontWeight: 600 }}>{qty} {t('lt_selected')}</span>
                  <button className="btn btn-ghost btn-sm" onClick={() => setSelected((p) => ({ ...p, [test.id]: qty + 1 }))}><Plus size={14} /></button>
                </div>
              ) : (
                <button className="btn btn-secondary btn-sm" onClick={() => setSelected((p) => ({ ...p, [test.id]: 1 }))}><Plus size={16} /> {t('lt_add_test')}</button>
              )}
            </div>
          )
        })}
      ...
      {bookings && bookings.length > 0 && (
        <div style={{ marginTop: 32 }}>
          <h3 style={{ marginBottom: 14 }}>{t('lt_your_bookings')}</h3>
            ...
            {bookings.slice(0, 5).map((b) => (
              <div key={b.id} className="card" style={{ padding: 16, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 150 }}>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>{b.test_ids.length} {b.test_ids.length > 1 ? t('lt_test_plural') : t('lt_test_singular')} - ₹{b.total}</div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{b.date} {b.home_collection && t('lt_home_collection_short')}</div>
                </div>
      ...
      <Modal open={checkoutOpen} onClose={() => setCheckoutOpen(false)} title={t('lt_modal_book_title')}
        footer={<><button className="btn btn-ghost" onClick={() => setCheckoutOpen(false)}>{t('lt_cancel')}</button>
          <button className="btn btn-primary" onClick={bookTests} disabled={!form.patient_name || !form.date || !form.time_slot || !paymentValid || paying}>
            <PayingButton paying={paying} label={`${t('lt_confirm_booking')} ₹${total}`} />
          </button></>}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ padding: 12, background: 'var(--neutral-50)', borderRadius: 'var(--radius-sm)', fontSize: 13 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, paddingTop: 8, marginTop: 8, borderTop: '1px solid var(--border)' }}><span>{t('lt_total')}</span><span>₹{total}</span></div>
          </div>
          <div><label className="label">{t('lt_patient_name')}</label><input className="input" value={form.patient_name} onChange={(e) => setForm({ ...form, patient_name: e.target.value })} /></div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div><label className="label">{t('lt_date')}</label><input className="input" type="date" min={today} value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /></div>
            <div><label className="label">{t('lt_time_slot')}</label>
              <select className="input" value={form.time_slot} onChange={(e) => setForm({ ...form, time_slot: e.target.value })}><option value="">{t('lt_select')}</option>
                <option>06:00-07:00</option><option>07:00-08:00</option><option>08:00-09:00</option><option>09:00-10:00</option></select>
            </div>
          </div>
          <div>
            <label className="label">{t('lt_sample_collection')}</label>
            <div style={{ display: 'flex', gap: 12 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14, cursor: 'pointer' }}><input type="radio" checked={form.home_collection} onChange={() => setForm({ ...form, home_collection: true })} /> <Home size={15} /> {t('lt_home_collection')}</label>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14, cursor: 'pointer' }}><input type="radio" checked={!form.home_collection} onChange={() => setForm({ ...form, home_collection: false })} /> {t('lt_visit_lab')}</label>
            </div>
          </div>
          {form.home_collection && <div><label className="label">{t('lt_address')}</label><textarea className="input" rows={2} value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} placeholder={t('lt_address_placeholder')} /></div>}
          <PaymentPanel method={paymentMethod} onMethodChange={setPaymentMethod} onValidChange={setPaymentValid} />
        </div>
      </Modal>

      <Modal open={success} onClose={() => setSuccess(false)} title={t('lt_booking_confirmed')}
        footer={<button className="btn btn-primary" onClick={() => setSuccess(false)}>{t('lt_done')}</button>}
      >
        <div style={{ textAlign: 'center', padding: 16 }}>
          ...
          <p>{t('lt_your_test')}{totalTests > 1 ? t('lt_test_plural') : ''} {form.home_collection ? t('lt_with_home') : ''} {form.home_collection ? t('lt_have_been_booked') : t('lt_has_been_booked')} <strong>{form.date}</strong>.</p>
        </div>
      </Modal>
    </div>
  )
