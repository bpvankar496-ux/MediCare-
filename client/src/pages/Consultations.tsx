...
export default function Consultations() {
  ...
  const bookConsultation = async () => {
    if (!form.doctor_name || !form.patient_name || !form.date || !form.time_slot) { setErr(t('cn_err_required')); return }
    setErr(null)
    ...
    <div className="fade-in">
      ...
      {incoming && (
        <div className="card fade-in" style={{ padding: 16, marginBottom: 20, background: 'var(--success-50)', border: '1px solid var(--success-100)', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 180 }}>
            <div style={{ fontWeight: 600, fontSize: 15 }}>{incoming.fromName} {t('cn_calling_you')}</div>
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>{t('cn_join_to_connect')}</div>
          </div>
          <button className="btn btn-primary btn-sm" onClick={() => {
            const c = consultations?.find((x) => x.id === incoming.consultationId)
            if (c) joinNow(c)
          }}>{t('cn_join_now')}</button>
          <button className="btn btn-ghost btn-sm" onClick={dismiss}>{t('cn_dismiss')}</button>
        </div>
      )}

      {permission !== 'granted' && (
        <button className="btn btn-secondary btn-sm" style={{ marginBottom: 20 }} onClick={request}>
          {t('cn_enable_notifications')}
        </button>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }} className="tele-grid">
        <div className="card" style={{ padding: 20, textAlign: 'center', cursor: 'pointer', transition: 'all 0.2s' }} onClick={() => { setForm({ ...form, mode: 'video' }); setModalOpen(true) }} onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-3px)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'none'}>
          ...
          <h4>{t('cn_video_title')}</h4>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>{t('cn_video_desc')}</p>
        </div>
        <div className="card" style={{ padding: 20, textAlign: 'center', cursor: 'pointer', transition: 'all 0.2s' }} onClick={() => { setForm({ ...form, mode: 'phone' }); setModalOpen(true) }} onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-3px)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'none'}>
          ...
          <h4>{t('cn_phone_title')}</h4>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>{t('cn_phone_desc')}</p>
        </div>
        <div className="card" style={{ padding: 20, textAlign: 'center', cursor: 'pointer', transition: 'all 0.2s' }} onClick={() => { setForm({ ...form, mode: 'chat' }); setModalOpen(true) }} onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-3px)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'none'}>
          ...
          <h4>{t('cn_chat_title')}</h4>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>{t('cn_chat_desc')}</p>
        </div>
      </div>

      {scheduled.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <h3 style={{ marginBottom: 14 }}>{t('cn_scheduled_title')}</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {scheduled.map((c) => (
              <div key={c.id} className="card" style={{ padding: 18, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                ...
                <button className="btn btn-primary btn-sm" onClick={() => joinNow(c)}>{t('cn_join_now')}</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {completed.length > 0 && (
        <div>
          <h3 style={{ marginBottom: 14 }}>{t('cn_past_title')}</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {completed.map((c) => (
              <div key={c.id} className="card" style={{ padding: 18 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                  <div>
                    ...
                    <span style={{ fontSize: 13, color: 'var(--text-muted)', marginLeft: 8 }}>{c.date} {t('cn_scheduled_for_3')} {c.time_slot}</span>
                  </div>
                  <span className="badge badge-success">{t('cn_completed')}</span>
                </div>
                {c.prescription && (
                  <div style={{ marginTop: 10, padding: 12, background: 'var(--neutral-50)', borderRadius: 'var(--radius-sm)', fontSize: 13 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                      <div><strong>{t('cn_prescription')}</strong> {c.prescription}</div>
                      <button
                ...
                {c.follow_up && <div style={{ marginTop: 8, fontSize: 13, color: 'var(--warning-600)' }}><CheckCircle size={14} style={{ display: 'inline', marginRight: 4 }} />{t('cn_follow_up')}</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      {scheduled.length === 0 && completed.length === 0 && (
        <EmptyState icon={Video} title={t('cn_empty_title')} subtitle={t('cn_empty_subtitle')} />
      )}

      <Modal open={modalOpen} onClose={() => { setModalOpen(false); setSuccess(false); setErr(null) }}
        title={success ? t('cn_modal_scheduled') : t('cn_modal_book_title')}
        footer={success ? <button className="btn btn-primary" onClick={() => { setModalOpen(false); setSuccess(false) }}>{t('cn_done')}</button>
          : <><button className="btn btn-ghost" onClick={() => setModalOpen(false)}>{t('cn_cancel')}</button>
            <button className="btn btn-primary" onClick={bookConsultation}>{t('cn_schedule')}</button></>}
      >
        {success ? (
          <div style={{ textAlign: 'center', padding: 16 }}>
            ...
            <p>{t('cn_scheduled_for')} <strong>{form.mode}</strong> {t('cn_scheduled_for_2')} <strong>{form.date}</strong> {t('cn_scheduled_for_3')} <strong>{form.time_slot}</strong>.</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <label className="label">{t('cn_select_doctor')}</label>
              <select className="input" value={form.doctor_name} onChange={(e) => setForm({ ...form, doctor_name: e.target.value })}>
                <option value="">{t('cn_choose_doctor')}</option>
                {doctors?.map((d) => <option key={d.id} value={d.name}>{d.name} - {d.specialty}</option>)}
              </select>
            </div>
            <div><label className="label">{t('cn_patient_name')}</label><input className="input" value={form.patient_name} onChange={(e) => setForm({ ...form, patient_name: e.target.value })} /></div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div><label className="label">{t('cn_date')}</label><input className="input" type="date" min={today} value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /></div>
              <div><label className="label">{t('cn_time')}</label><select className="input" value={form.time_slot} onChange={(e) => setForm({ ...form, time_slot: e.target.value })}><option value="">{t('cn_select')}</option><option>10:00-10:30</option><option>11:00-11:30</option><option>14:00-14:30</option><option>16:00-16:30</option></select></div>
            </div>
            <div><label className="label">{t('cn_mode')}</label><select className="input" value={form.mode} onChange={(e) => setForm({ ...form, mode: e.target.value })}><option value="video">{t('cn_mode_video')}</option><option value="phone">{t('cn_mode_phone')}</option><option value="chat">{t('cn_mode_chat')}</option></select></div>
            <div><label className="label">{t('cn_symptoms')}</label><textarea className="input" rows={2} value={form.symptoms} onChange={(e) => setForm({ ...form, symptoms: e.target.value })} placeholder={t('cn_symptoms_placeholder')} /></div>
      ...
      {activeCall && activeCall.mode === 'chat' && (
        <Modal open={true} onClose={() => setActiveCall(null)} title={`${t('cn_chat_with')} ${activeCall.doctor_name}`}
          footer={<button className="btn btn-ghost" onClick={() => setActiveCall(null)}>{t('cn_close')}</button>}
        >
